...
@pytest.mark.parametrize("ossie_dialect", list(OSIDialect))
def test_preserves_enum(ctx: ExportContext, ossie_dialect: OSIDialect) -> None:
...
@pytest.mark.parametrize("ossie_dialect", list(OSIDialect))
def test_parses_valid_string(ctx: ExportContext, ossie_dialect: OSIDialect) -> None:
...
@pytest.mark.parametrize("ossie_dialect", list(OSIDialect))
def test_parses_lowercase_string(ctx: ExportContext, ossie_dialect: OSIDialect) -> None:
...
def test_warns_and_defaults_for_invalid_string(ctx: ExportContext) -> None:
    ...
    result = load_ossie_dialect(ossie_dialect, ctx=ctx)
    assert result is OSIDialect.ANSI_SQL
