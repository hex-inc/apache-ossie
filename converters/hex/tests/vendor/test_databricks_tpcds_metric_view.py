...
FIXTURE = VENDOR_ROOT / "databricks" / "tests" / "fixtures" / "tpcds_metric_view.yaml"


def test_databricks_tpcds_metric_view_to_hex(
    snapshot: SnapshotAssertion,
    tmp_path: Path,
) -> None:
    assert_vendor_to_hex(
        convert_metric_view_to_ossie(read_text(FIXTURE)),
        dialect="databricks",
        snapshot=snapshot,
        tmp_path=tmp_path,
    )
